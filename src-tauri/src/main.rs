...
#[tauri::command]
fn update_plan(app: tauri::AppHandle, state: State<'_, AppState>, req: PlanUpdateRequest) -> Result<plan::Plan, String> {
...
#[tauri::command]
async fn start_session(app: tauri::AppHandle, state: State<'_, AppState>, req: StartSessionRequest) -> Result<String, String> {
  state
    .sessions
...
#[tauri::command]
async fn send_input(state: State<'_, AppState>, session_id: String, data: String) -> Result<(), String> {
  state.sessions.send(&session_id, data).await.map_err(|e| e.to_string())
}

#[tauri::command]
async fn stop_session(state: State<'_, AppState>, session_id: String) -> Result<(), String> {
  state.sessions.stop(&session_id).await.map_err(|e| e.to_string())
}

#[tauri::command]
fn resize_view(_session_id: String, _cols: u32, _rows: u32) -> Result<(), String> { Ok(()) }
...
#[derive(Serialize)]
struct ApplyPatchResponse { summary: String }

#[tauri::command]
fn apply_patch(_state: State<'_, AppState>, req: ApplyPatchRequest) -> Result<ApplyPatchResponse, String> {
  ...
  if !has_begin || !has_end { return Err("invalid patch format".into()); }
  Ok(ApplyPatchResponse { summary: "validated patch (no-op)".into() })
}
